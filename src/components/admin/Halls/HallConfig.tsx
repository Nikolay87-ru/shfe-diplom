...
const ROWS_MIN = 1, ROWS_MAX = 50, PLACES_MIN = 1, PLACES_MAX = 50;
...
export const HallConfig: React.FC = () => {
  ...
  const [initial, setInitial] = useState<{rows:number,places:number,config:string[][]}|null>(null);
  ...
  function handleSizeApply(e: React.FormEvent) {
    ...
    setConfig(Array.from({length: rows}, ()=>Array.from({length: places}, _=>"standart")));
    setChanged(true);
  }
  ...
  return (
    <section className="admin__section hall-config">
      <div className="admin__header">
        <h2 className="admin__header_text">Конфигурация залов</h2>
        {/* стрелка */}
      </div>
      <div className="admin__wrapper">
        ...
        <div className="hall-config__wrapper">
          ...
          <form className="hall-config__size" onSubmit={handleSizeApply}>
            ...
            <button type="submit" className="button" style={{marginLeft:12}}>Применить</button>
          </form>
          ...
          <div className="hall-config__hint">
          ...
          <div className="hall-config__buttons">
            <button
              className={"admin__button_cancel hall-config__batton_cancel button"+
            ...
            <button
              className={"admin__button_save hall-config__batton_save button"+
                (!changed?" button_disabled":"")}
              onClick={handleSave}
              disabled={!changed}
            >
