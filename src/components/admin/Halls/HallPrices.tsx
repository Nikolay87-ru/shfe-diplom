...
export const HallPrices: React.FC = () => {
  ...
  function handleCancel(e:React.MouseEvent) {
    e.preventDefault();
    if(!initial) return;
    setPriceSt(initial.standart); setPriceVip(initial.vip); setChanged(false);
  }

  async function handleSave(e: React.MouseEvent) {
    e.preventDefault(); if(!hall) return;
  ...
  return (
    <section className="admin__section price-config">
      <div className="admin__header">
        <h2 className="admin__header_text">Конфигурация цен</h2>
      </div>
      <div className="admin__wrapper">
        ...
        <div className="price-config__wrapper">
          ...
          <div className="price-config__buttons">
            <button
              className={"admin__button_cancel price-config__batton_cancel button"
            ...
            >Отмена</button>
            <button
              className={"admin__button_save price-config__batton_save button"
                +(changed?"":" button_disabled")}
              onClick={handleSave}
              tabIndex={0}
              disabled={!changed}
