...
const server = setupServer(
  ...
  http.delete(`${API_URL}/hall/:id`, async ({ params }) => {
    const { id } = params
    const index = mockData.halls?.findIndex(h => h.id === Number(id)) ?? -1
  ...
  http.delete(`${API_URL}/film/:id`, async ({ params }) => {
    const { id } = params
    const index = mockData.films?.findIndex(f => f.id === Number(id)) ?? -1
  ...
  http.delete(`${API_URL}/seance/:id`, async ({ params }) => {
    const { id } = params
    const index = mockData.seances?.findIndex(s => s.id === Number(id)) ?? -1
